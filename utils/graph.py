...
def generate_random_cycle_plus_graph(
    cycle_length, aug_nodes=3, sample_size=10, gen_p=0.2
):
    ...
    for i in range(sample_size):
        ...
        for j in aug_nodes_ind:
            p = np.random.binomial(1, p=np.zeros(num_nodes) + gen_p).nonzero()[
                0
            ]
            for k in p:
                aug_edge.append([j, k])
    ...
    for g in cycle_graphs:
        cg = construct_graph_from_edges(
            g[:, 0], g[:, 1], n_entities=num_nodes, inverse_edge=True
        )
        data.append([cg, 1])
...
def generate_random_grid_graph(
    cycle_length, aug_nodes=3, sample_size=10, gen_p=0.2
):
...
def generate_randomvv_grid_graph(
    cycle_length, aug_nodes=3, sample_size=10, gen_p=0.2
):
    a_edges = [[0, 1], [0, 2], [1, 3], [2, 3], [2, 4], [3, 5], [4, 5]]
    b_edges = [[0, 1], [0, 2], [1, 2], [2, 3], [3, 4], [3, 5], [4, 5]]
...
def sample_subgraph_around_link(
    adj, head, tail, max_dist, sample_size, max_nodes_per_hop
):
    ...
    neighbors = sample_fixed_hop_size_neighbor(
        ...
        max_nodes_per_hope=max_nodes_per_hop,
    )
    ...
    gt_g.add_edge_list(
        np.concatenate([row.reshape(-1, 1), col.reshape(-1, 1)], axis=-1)
    )
...
def sample_subgraph_around_link_dgl(
    adj, graph, head, tail, max_dist, sample_size, max_nodes_per_hop
):
    neighbors = sample_fixed_hop_size_neighbor(
        adj,
        [head, tail],
