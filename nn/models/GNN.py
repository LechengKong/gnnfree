...
class MultiLayerMessagePassingVN(MultiLayerMessagePassing):
    def __init__(self, num_layers, inp_dim, out_dim, layer_t=GINLayer, drop_ratio=0, JK='last', batch_norm=True):
        ...
        for layer in range(self.num_layers-1):
            self.virtualnode_mlp_list.append(MLPLayers(2, h_units=[self.out_dim, 2*self.out_dim, self.out_dim]))
    ...
    def forward(self, g):
        ...
        vnode_embed = self.virtualnode_embedding(torch.zeros(g.batch_size, dtype=torch.int).to(g.device))

        batch_node_segment = torch.arange(g.batch_size, dtype=torch.long, device=g.device).repeat_interleave(g.batch_num_nodes())
        ...
        for layer in range(self.num_layers):
            ...
            if layer < self.num_layers - 1:
                vnode_emb_temp = scatter(h, batch_node_segment, dim=0, dim_size = g.batch_size) + vnode_embed

                vnode_embed = F.dropout(self.virtualnode_mlp_list[layer](vnode_emb_temp), self.drop_ratio, training=self.training)
...
class HomogeneousGNN(MultiLayerMessagePassing):
    ...
    def build_layers(self):
        for layer in range(self.num_layers):
            if layer == 0:
                self.conv.append(self.layer_t(self.inp_dim, self.out_dim))
            else:
                self.conv.append(self.layer_t(self.out_dim, self.out_dim))
...
class HomogeneousEdgeGNN(MultiLayerMessagePassing):
    def __init__(self, num_layers, inp_dim, out_dim, edge_dim, layer_t=GINELayer, drop_ratio=0, JK='last', batch_norm=True):
        super().__init__(num_layers, inp_dim, out_dim, layer_t, drop_ratio, JK, batch_norm)
    ...
    def build_layers(self):
        for layer in range(self.num_layers):
            if layer == 0:
                self.conv.append(GINELayer(self.inp_dim, self.out_dim, self.edge_dim))
            else:
                self.conv.append(GINELayer(self.out_dim, self.out_dim, self.edge_dim))
...
class RGCN(MultiLayerMessagePassing):
    def __init__(self, num_layers, num_rels, inp_dim, out_dim, num_bases=4, layer_t=RelGraphConv, drop_ratio=0, JK='last', batch_norm=True):
        super().__init__(num_layers, inp_dim, out_dim, layer_t, drop_ratio, JK, batch_norm)
    ...
    def build_layers(self):
        for layer in range(self.num_layers):
            if layer == 0:
                self.conv.append(RelGraphConv(self.inp_dim, self.out_dim, self.num_rels, num_bases=self.num_bases, activation=F.relu))
            else:
                self.conv.append(RelGraphConv(self.out_dim, self.out_dim, self.num_rels, num_bases=self.num_bases, activation=F.relu))
