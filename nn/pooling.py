...
class GDPool(Pooler):
    ...
    def forward(
        ...
        combined_gd_repr = scatter(
            combined_gd_repr,
            torch.arange(len(gd_count), device=repr.device).repeat_interleave(
                gd_count
            ),
            dim=0,
        ...
        combined_repr = scatter(
            combined_repr,
            torch.arange(
                len(neighbor_count), device=repr.device
            ).repeat_interleave(neighbor_count),
            dim=0,
...
class HorGDExtractor(LinkReprExtractor):
    def forward(self, repr, geodesic, gd_len):
        ...
        gd_repr = scatter(
            gd_repr,
            torch.arange(len(gd_len), device=repr.device).repeat_interleave(
                gd_len
            ),
            dim=0,
            dim_size=len(gd_len),
...
class VerGDExtractor(LinkReprExtractor):
    def __init__(self, emb_dim, gd_deg=False) -> None:
        ...
        self.mlp_head_gd_process = MLPLayers(
            2, h_units=[emb_dim, 2 * emb_dim, emb_dim]
        )
        self.mlp_tail_gd_process = MLPLayers(
            2, h_units=[emb_dim, 2 * emb_dim, emb_dim]
        )
    ...
    def forward(
        ...
        head_gd_repr = self.get_ver_gd_one_side(
            repr, head_gd, head_gd_len, head_gd_deg
        )
        tail_gd_repr = self.get_ver_gd_one_side(
            repr, tail_gd, tail_gd_len, tail_gd_deg
        )
        return self.mlp_head_gd_process(
            head_gd_repr
        ) + self.mlp_tail_gd_process(tail_gd_repr)
...
feature_module_dict = {
    "node_pair": NodePairExtractor,
    "rel": RelExtractor,
    "HorGD": HorGDExtractor,
