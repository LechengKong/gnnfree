...
class Manager:
    ...
    def train(
        ...
        for epoch in range(
            self.starting_epoch + 1, self.starting_epoch + num_epochs + 1
        ):
            print("Epoch", epoch)
            self.current_epoch = epoch
            print("Train {}".format(train_learner.name))
            train_metric = trainer.train_scheduled(
                train_learner, optimizer, device
            )
            print(train_metric)
            if (
                epoch % eval_every == 0
                or epoch == self.starting_epoch + num_epochs
            ):
                print("Eval {}".format(val_learner.name))
                metrics = trainer.eval_scheduled(val_learner, device)
                print(metrics)
                update = trainer.eval_metric(metrics[metric_name], best_res)
                if update:
                    print("Found better model")
                    best_res = metrics[metric_name]
    ...
    def eval(self, learner, trainer, device=None):
        print("Eval {}".format(learner.name))
